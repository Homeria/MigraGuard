...
type WorkloadProfiler interface {
	CalculateTPS(t time.Time, simulationStart time.Time, profile types.SQLiteHistoryProfile) float64
}
...
func (p *DefaultWorkloadProfiler) CalculateTPS(t time.Time, simulationStart time.Time, profile types.SQLiteHistoryProfile) float64 {
	...
	eventMult := 1.0
	eventBaseDay := time.Date(simulationStart.Year(), simulationStart.Month(), simulationStart.Day(), 0, 0, 0, 0, simulationStart.Location())
	for _, event := range profile.Events {
		eventStart := eventBaseDay.
			AddDate(0, 0, event.StartDay).
			Add(time.Duration(event.StartHour) * time.Hour)
		eventEnd := eventStart.Add(time.Duration(event.DurationH) * time.Hour)

		if !t.Before(eventStart) && t.Before(eventEnd) {
